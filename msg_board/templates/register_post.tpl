...
<body>
    ...
    {elseif ! isset($name)}
		...
		{return}
    {elseif ! isset($nname)}
        <script> swal({ title: "註冊失敗", text: "暱稱不得空白，請重新輸入！", icon: "error", button: "確定 " }).then(function() { history.go(-1); }); </script>
		{return}
	{elseif ! isset($gender)}
        <script> swal({ title: "註冊失敗", text: "請選擇性別！", icon: "error", button: "確定 " }).then(function() { history.go(-1); }); </script>
		{return}
    {elseif ! isset($phone)}
	...
	{* 確認帳號是否註冊成功，並顯示結果 *}
	{if $success}
        <script> swal({ title: "註冊成功", icon: "success", button: "返回主畫面 " }).then(function() { window.location = '{$script.menu}'; }); </script>
	{else}
