<?php 
	...
	$smt->assign('title','登入');  // title : 此網頁頁面名稱
	...
	if($input_verification == TRUE)
	{
		...
		$sql = "select * from member where ID_number='".mysqli_real_escape_string($link,$id)."' limit 1";
		$result = mysqli_query($link,$sql);
		$num = mysqli_fetch_array($result);

		if(mysqli_num_rows($result)==1)
			if(password_verify($pd,$num['Password']))  // 密碼解密
			{
				// 判斷帳號是否鎖定 
				if($num['Locked'] == 1)
				{
				...
				else
				{
					...
					$_SESSION['Nickname'] = $num['Nickname'];
